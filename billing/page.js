          ...
          <>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:16}}>
              {PLANS.map(plan => {
                ...
                return (
                  <div key={plan.id} style={{borderRadius:14,border: plan.highlight ? '2px solid #0284c7' : isCurrent ? '2px solid #059669' : '1px solid #e2e8f0',padding:22,background: plan.highlight ? '#f5f3ff' : isCurrent ? '#f0fdf4' : '#fff',position:'relative'}}>
                    {plan.highlight && !isCurrent && <div style={{position:'absolute',top:-12,left:'50%',transform:'translateX(-50%)',background:'#0284c7',color:'#fff',fontSize:10,fontWeight:700,padding:'3px 12px',borderRadius:10,whiteSpace:'nowrap'}}>Most Popular</div>}
                    ...
                    <button onClick={() => handleUpgrade(plan.id)} disabled={isCurrent || plan.id === 'free' || loading}
                      style={{width:'100%',padding:'9px',borderRadius:8,border:'none',
                        background: isCurrent ? '#059669' : plan.highlight ? '#0284c7' : '#f8fafc',
                        color: isCurrent || plan.highlight ? '#fff' : '#0284c7',
                    ...
                    <div style={{borderTop:'1px solid #e2e8f0',paddingTop:12}}>
                      {plan.features.map(f => (
                        <div key={f} style={{display:'flex',gap:8,marginBottom:6}}>
                          <span style={{color:'#0284c7',fontSize:12}}>✓</span>
            ...
            {invoices.length === 0 ? (
              ...
              <div key={i} style={{display:'flex',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid #f8fafc',fontSize:13}}>
                ...
                <div style={{display:'flex',gap:12,alignItems:'center'}}>
                  ...
                  {inv.pdf && <a href={inv.pdf} target="_blank" rel="noopener noreferrer" style={{color:'#0284c7',fontSize:11,fontWeight:600,textDecoration:'none'}}>↓ PDF</a>}
                </div>
              </div>
            ))}
          </div>
