...
useEffect(() => {
  ...
  useEffect(() => {
    const match = document.cookie.match(/pri_user=([^;]+)/)
  ...
  const loadItems = async (userId) => {
    try {
      const token = document.cookie.match(/pri_token=([^;]+)/)?.[1] || ''
  ...
  const handleLogout = () => {
    document.cookie = 'pri_token=; path=/; max-age=0'
    document.cookie = 'pri_user=; path=/; max-age=0'
    router.push('/login')
  }
  ...
  const Sidebar = () => (
    <div style={{width:220,background:'#0f172a',display:'flex',flexDirection:'column',minHeight:'100vh',flexShrink:0,position:'relative'}}>
        ...
        {usageData && usageData.plan === 'free' && (
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:16}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
              ...
              <span style={{fontSize:12,fontWeight:700,color:usageData.used >= usageData.limit ? '#dc2626' : '#0284c7'}}>{usageData.used} / {usageData.limit} uses</span>
            </div>
            <div style={{background:'#f1f5f9',borderRadius:6,height:6,marginBottom:8}}>
              <div style={{background:usageData.used >= usageData.limit ? '#dc2626' : '#0284c7',borderRadius:6,height:6,width:Math.min(100,(usageData.used/usageData.limit)*100)+'%',transition:'width 0.3s'}}/>
            </div>
            {usageData.used >= 2 && (
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                ...
                <a href="/billing" style={{fontSize:11,fontWeight:700,color:'#0284c7',textDecoration:'none'}}>Upgrade →</a>
              </div>
            )}
          </div>
        )}
      ...
      <div style={{padding:'12px 12px 8px'}}>
        <Link href="/generate" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:7,background:'#0284c7',color:'#fff',textDecoration:'none',borderRadius:8,padding:'10px',fontSize:13,fontWeight:600,letterSpacing:'-0.2px'}}>
          <span style={{fontSize:16,lineHeight:1}}>+</span> New Monitor
        </Link>
      </div>
      ...
      <div style={{padding:'4px 8px',flex:1}}>
        {[
          ...
          { icon:'□', label:'Monitors', href:'/items' },
      ...
      <div style={{padding:'12px',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
        <div style={{background:'rgba(255,255,255,0.04)',borderRadius:10,padding:'10px 12px',marginBottom:8,border:'1px solid rgba(255,255,255,0.06)'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}>
            ...
            <Link href="/billing" style={{fontSize:10,color:'#0284c7',textDecoration:'none',fontWeight:600}}>Upgrade</Link>
          </div>
          <div style={{background:'rgba(255,255,255,0.08)',borderRadius:3,height:3,marginBottom:6}}>
            <div style={{background:'#0284c7',borderRadius:3,height:3,width:`${Math.min(100,(items.length/3)*100)}%`,transition:'width 0.3s'}}/>
          </div>
        ...
        <button onClick={handleLogout}
          ...
          <div style={{width:28,height:28,borderRadius:7,background:'#0284c7',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#fff',flexShrink:0}}>
            {getInitials(user)}
          </div>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
        ...
        <div style={{background:'#fff',borderBottom:'1px solid #f1f5f9',height:56,display:'flex',alignItems:'center',padding:'0 20px',gap:12,flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            ...
            <p style={{fontSize:11,color:'#94a3b8',marginTop:1}}>Here's your PriceSignal overview.</p>
          </div>
          <Link href="/generate"
            style={{background:'#0284c7',color:'#fff',padding:'7px 16px',borderRadius:8,fontSize:13,fontWeight:600,textDecoration:'none',letterSpacing:'-0.2px',flexShrink:0}}>
            + New
          </Link>
        </div>
        ...
        <div style={{flex:1,padding:'20px',overflowY:'auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)',gap:12,marginBottom:20}}>
            {[
              { label:'Total', value: items.length, sub: 'All time', color:'#0284c7', icon:'📄' },
        ...
        {usageData && usageData.plan === 'free' && (
          <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:12,padding:16,marginBottom:16}}>
            ...
            <div style={{background:'#f1f5f9',borderRadius:6,height:6,marginBottom:8}}>
              <div style={{background:usageData.used >= usageData.limit ? '#dc2626' : '#0284c7',borderRadius:6,height:6,width:Math.max(4,Math.min(100,(usageData.used/usageData.limit)*100))+'%',transition:'width 0.3s'}}/>  
            </div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontSize:11,color:'#94a3b8'}}>Resets {new Date(usageData.reset_date).toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>
              <a href="/billing" style={{fontSize:11,fontWeight:700,color:'#0284c7',textDecoration:'none'}}>Upgrade →</a>
            ...
            <div style={{background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',overflow:'hidden',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
              <div style={{padding:'16px 20px',borderBottom:'1px solid #f8fafc',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <p style={{fontSize:14,fontWeight:700,color:'#0f172a',letterSpacing:'-0.2px'}}>Recent Monitors</p>
                <Link href="/generate" style={{fontSize:12,color:'#0284c7',textDecoration:'none',fontWeight:600,background:'rgba(0,0,0,0.03)',padding:'4px 10px',borderRadius:6}}>+ New</Link>
              </div>
              ...
              {loading ? (
                ...
                <div style={{padding:'48px 24px',textAlign:'center'}}>
                  ...
                  <p style={{fontSize:14,fontWeight:600,color:'#0f172a',marginBottom:6}}>No Monitors yet</p>
                  <p style={{fontSize:13,color:'#94a3b8',marginBottom:20}}>Generate your first one in seconds</p>
                  <Link href="/generate" style={{background:'#0284c7',color:'#fff',padding:'9px 20px',borderRadius:8,fontSize:13,fontWeight:600,textDecoration:'none'}}>
                    Get started →
                  </Link>
                </div>
                ...
                <div key={item.id}
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:12,minWidth:0,flex:1}}>
                    <div style={{width:34,height:34,borderRadius:9,background:'#f0f4ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:'#0284c7',flexShrink:0,border:'1px solid rgba(0,0,0,0.04)'}}>
                  ...
                  <Link href={`/items/${item.id}`}
                    style={{color:'#0284c7',fontSize:11,fontWeight:600,textDecoration:'none',padding:'5px 10px',border:'1px solid rgba(0,0,0,0.08)',borderRadius:6,flexShrink:0,marginLeft:12,background:'#fff'}}>
                    View →
                  </Link>
                </div>
              ))}
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              <div style={{background:'#fff',borderRadius:14,border:'1px solid #f1f5f9',padding:20,flex:1,boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                ...
                {items.length === 0 ? (
                  ...
                  <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,marginBottom:12}}>
                    <div style={{width:26,height:26,borderRadius:'50%',background:'#f0f4ff',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,flexShrink:0,color:'#0284c7',fontWeight:700,border:'1px solid rgba(0,0,0,0.04)'}}>+</div>
                    <div style={{minWidth:0}}>
              ...
              {items.length >= 2 && (
                <div style={{background:'#fafafa',borderRadius:14,border:'1px solid #f1f5f9',padding:20}}>
                  ...
                  <p style={{fontSize:12,color:'#64748b',marginBottom:12,lineHeight:1.5}}>Get unlimited Monitors from $19/month.</p>
                  <Link href="/billing" style={{display:'block',textAlign:'center',background:'#0284c7',color:'#fff',textDecoration:'none',borderRadius:8,padding:'8px',fontSize:12,fontWeight:700}}>
                    View plans →
                  </Link>
                </div>
              )}
