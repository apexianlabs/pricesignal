...
export async function POST(request) {
  try {
    ...
    // Check usage limits
    if (userId) {
      const usageRes = await fetch(`${process.env.DB_API_URL}/usage/check?user_id=${userId}&product=pricesignal`, {
        headers: { 'Authorization': `Bearer ${process.env.DB_API_KEY_PRICESIGNAL}` }
      })
    ...
    if (userId) {
      fetch(`${process.env.DB_API_URL}/usage/track`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.DB_API_KEY_PRICESIGNAL}` },
        body: JSON.stringify({ user_id: userId, product: 'pricesignal', action: 'generate' })
      }).catch(() => {})
    }
