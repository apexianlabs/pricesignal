...
export async function POST(request) {
  try {
    ...
    const aiRes = await fetch(`${AI_API_URL}/api/process`, {
      ...
      body: JSON.stringify({ task: 'summarise_changes', inputs })
    })
    ...
    if (userId && DB_API_URL) {
      try {
        const dbRes = await fetch(`${DB_API_URL}/db/pricesignal/monitors`, {
          ...
          body: JSON.stringify({ user_id: userId, title: `${company_name} pricing`, url, company_name, result_data: result, status: 'complete' })
        })
