...
const Logo = ({ size = 32 }) => (
  <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    <defs><linearGradient id="lg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#38bdf8"/><stop offset="100%" stopColor="#0369a1"/></linearGradient></defs>
    <path d="M15 15 L58 15 L85 42 L58 85 L15 85 Z" fill="url(#lg)"/>
    <circle cx="36" cy="36" r="7" fill="white" opacity="0.9"/>
    <path d="M28 68 L36 54 L47 62 L60 44 L72 52" stroke="white" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none" opacity="0.9"/>
  </svg>
)


const GOOGLE_AUTH_URL = `https://wdeilbhnsdlmckovicqy.supabase.co/auth/v1/authorize?provider=google&redirect_to=https%3A%2F%2Fapp.pricesignal.io%2Fauth%2Fcallback&scopes=email+profile`
...
export default function LoginPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const body = tab === 'login'
        ? { email: form.email, password: form.password, product: 'pricesignal' }
        : { email: form.email, password: form.password, full_name: form.name, source_product: 'pricesignal' }
      ...
      document.cookie = `pri_token=${token}; path=/; max-age=2592000; SameSite=Lax`
      document.cookie = `pri_user=${encodeURIComponent(JSON.stringify({ id: user.id, email: user.email, name: fullName }))}; path=/; max-age=2592000; SameSite=Lax`
      ...
      if (tab === 'signup') {
        fetch(`${process.env.NEXT_PUBLIC_DB_API_URL}/email/welcome`, {
          ...
          body: JSON.stringify({ product: 'pricesignal', to: form.email, name: fullName, email: form.email })
  ...
  const leftPanel = (
    <div style={{width:420,flexShrink:0,background:'#0284c7',display:'flex',flexDirection:'column',padding:'40px',position:'relative',overflow:'hidden'}}>
      ...
      <Link href="/" style={{display:'flex',alignItems:'center',gap:10,marginBottom:48,zIndex:1,textDecoration:'none'}}>
        <Logo size={36}/>
        <span style={{fontSize:20,fontWeight:800,color:'#fff'}}>PriceSignal</span>
      </Link>
      <div style={{zIndex:1,marginBottom:40}}>
        ...
        <p style={{fontSize:14,color:'rgba(255,255,255,0.7)',lineHeight:1.6}}>Track competitor pricing pages and get instant alerts when prices change.</p>
      </div>
      <div style={{zIndex:1,marginTop:'auto',background:'rgba(255,255,255,0.1)',borderRadius:12,padding:20,border:'1px solid rgba(255,255,255,0.15)'}}>
        <p style={{fontSize:13,color:'rgba(255,255,255,0.9)',lineHeight:1.6,marginBottom:10,fontStyle:'italic'}}>"PriceSignal saves me hours every week."</p>
        ...
        <div style={{width:'100%',maxWidth:400}}>
          {isMobile && (
            <Link href="/" style={{display:'flex',alignItems:'center',gap:8,marginBottom:24,textDecoration:'none',justifyContent:'center'}}>
              <Logo size={36}/>
              <span style={{fontWeight:800,fontSize:18,color:'#0f172a'}}>PriceSignal</span>
            </Link>
          )}
          ...
          <p style={{fontSize:14,color:'#64748b',marginBottom:20}}>
            {tab === 'login' ? `Sign in to your PriceSignal account.` : 'Start your free trial today.'}
          </p>
          ...
          <button onClick={handleSubmit} disabled={loading}
            style={{width:'100%',padding:13,borderRadius:10,border:'none',background:'#0284c7',color:'#fff',fontSize:15,fontWeight:700,cursor:'pointer',fontFamily:'Inter,sans-serif',marginBottom:16,opacity:loading?0.7:1}}>
